...
``
  g++ -std=c++11 -o oop24 oop24.cpp
  ./oop24
``
Χρειαζόμαστε αυτου του τυπου μεταγλωτηση γιατι τα λήμματα (lambdas) που χρησιμοποιηθηκαν σε καποια σημεια του κωδικα, εισήχθησαν στο C++11, και χωρίς το -std=c++11, ο compiler δεν μπορεί να τα αναγνωρίσει.

 *Περιγραφη*
  Το προγραμμα ξεκινα με την κλαση Product. Εντος της κλασης τα προστατευομενα μελοι ειναι τα προιοντα. Τα δημοσια μελοι ειναι ο constructor της κλαση ωστε να δημιουργουνται αντικειμενα της κλασης, στην συνεχεια εχουμε ορισει την συναρτηση ``DisplayProductInfo()`` για εμαφανιση των χαρακτηριστικων του προιοντος. Επειτα υπαρχουν  getrs και setrs για το καθε χαρακτηριστικο του προιοντος. Τελος για αυτην την κλαση εχουμε ορισει εναν seter για αλλαγη της συνολικης ποσοτητας του συγκεκριμενου προιοντος απο ολους τους χρηστες και υπαρχει και ο αντιστοιχος geter για επιστροφη της ποσοτητας.
  ...
  ``
  Μετα απο την παραπανω κλαση χρησιμοποιησουμε ενα struct που περιεχει εναν operator τυπου bool ωστε να χρησιμοποιηθει παρακατω στην ταξινομηση των προιοντων με βαση τις συνολικες τους πωλησεις ωστε να βρεθουν τα 5 best seller ποιοντα.
...
``
  ...
  Εχουμε ορισει την κλαση ``Cart`` η οποια περιεχει στα προστατεομενα μελοι ενα struct ``Item`` που εχει ενα αντικειμενο τυπου ``Product`` την ποσοτητα του προιοντος που θελει ο πελατης να βαλει στο καλαθι του. Επειτα υπαρχει το ενα vector απο τα προιοντα που βρισκονται στο καλαθι, το συνολικο κοστος του καλαθιου και το ονομα του πελατη. Ως public εχουμε δηλωσει εναν μετρητη για να βρισκουμε το πληθος των καλαθιων, Εναν default constructor της κλασης Cart, εναν constructor της cart που δεχεται σαν ορισμα το ονομα του πελατη και αρχικοποιει με μηδεν το συνολικο κοστος. Στην συνεχεια εχουμε προσθεσει geters και seters για το ονομα του πελατη το συνολικο κοστος και την ποσοτητα (για την ποσοτητα μονο geter). Εχουμε ορισει την συναρτηση ``addItem`` για προσθηκη ενος προιοντος στο καλαθι. Αν το προιον βρισκεται ειδη μεσα στο καλαθι τοτε προσθετουμε την ποστητα που θελει ο πελατης (ελεγχουμε και αν η ποσοτητα που θελει ο πελατης ειναι διαθεσιμη στο καταστημα). Αν δεν υπαρχει το προιον στο καλαθι τοτε το προσθετουμε. Με την ιδια λογικη λειτουργει και η συναρτηση ``removeItem`` απλα εδω αντι να προσθετω οπως κανω στην  ``addItem`` αφαιρω. Η συναρτηση ``checkout`` χρησιμοποιειται για να εμφανιζει το καλαθι, να αποθηκευει το ιστορικο παραγγελιων και τελος να καθαριζει το καλαθι. Δηλαδη με απλα λογια χρησιμοποιηται για να ολοκληρωνει ο πελατης την παραγγελια του. Η  ``Save_order_history()`` αποθηκευει μεσα στο αρχειο ``cartOwner_history.txt`` το ιστορικο παραγγελιων του πελατη.
...
``
    ...
    void checkout() {
        ...
        save_order_history();   // ιστορικο παραγγελιων
...
``
 ...
 Η κλαση ``user `` περιεχει ως proteceted το username, το password και το isAdmin. Ως public περιεχει το cosnstructor της κλασης, στην συνεχεια εχουμε υλοποιηση την συναρτηση ``login`` για συνδεση του χρηστη στο λογαριασμο του. Επειτα η συναρτηση ``fuctionsearch`` ελεγχει αν υπαρχει ο χρηστης στο αρχειο users.txt.
...
``
  ...
  Η κλαση ``Admin`` κληρονομειται απο την κλαση ``User``. Περιεχει τον costructor για την δημιουργια αντικειμεων. Η κλαση περιεχει την συναρτηση ``displayOptions`` που δειχνει στον Admin τις επιλογες που εχει. Η ``viewAllProducts`` εμφανιζει τα διαθεσιμα προιοντα του καταστηματος μαζι με την παεριγραφη τους. Επισης ο Admin εχει την δυνατοτητα μεσω της συναρτησης ``addProduct`` να προσθεσει καποιο προιον στο καταστημα μαζι με τα χαρακτηριστικα του καθως και να αλλαξει την περιγραφη, την τιμη και την ποσοτητα ενος προιοντος χρησιμοποιωντας την συναρτηση ``changeProduct``. Επισης εχει την ευκαιρια να αναζητησει ενα προιον στο καταστημα με βαση τον τιτλο του, την κατηγορια του ή την υποκατηγορια του. Η συναρτηση ``viewOutOfStockProducts`` του δειχνει τα προιοντα που εχουν εξαντληθει. Μια ακομη λειτουργια αυτης της κλασης ειναι να εμφανιζονται τα 5 κορυφαια προιοντα του καταστηματος με βαση της πωλησεις. Ολες οι παραπανω λειτουργιες υλοποιουνται με την συναρτηση ``menu``.
  ...
  Η κλαση ``Customer`` που κληρονομηται απο την κλαση ``User`` δειχνει στο πελατη μεσω της συναρτησης ``displayOptions`` τις επιλογες που εχει. Αρχικα δεινει την δυνατοτητα να αναζητησει ενα προιον με βαση το ονομα του, την κατηγορια του ή την υποκατηγορια του η λειτουργια αυτην υλοποοηται με την συναρτηση ``searchProduct`` . Επισης μπορει να προσθεσει ενα προιον στο καλαθι του αν αυτο υπαρχει χρησιμοποιωντας την συναρτηση ``addProduct``. Αντιστοιχα μπορει και να αφαιρεση ενα προιον απο το καλαθι των αγορων του με την συναρτηση ``removeProduct``. Μπορει να ολοκληρωσει την παραγγελια του με την συναρτηση ``checkout``. Καθως μπορει να δει και το ιστορικο των παραγγελιων του απο την συναρτηση ``view_order_history``. Ολες αυτες οι λειτουργιες υλοποιουνται με την συναρτηση `` menu``.
  ...
  ``
    ...
    void addProduct(vector<Product>& products){
        ...
        cin.ignore();
        getline(cin, title);
        cout << title << endl;  // πληκτρολογει το προιον
        for(Product& p : products){
    ...
    void removeProduct(vector<Product>& products){
        ...
        for(Product& p : products){
            ...
            if(p.getTitle() == title){
                ...
                cout << "PRODUCTS LEDT POSOTITA IS : " << p.getQuantity() << endl;
  ...
  Η συναρτησ signup δεινει την συνατοτηα στο χρηστη να φτιαξει λογαριασμο στο καταστημα ειτε ειναι πελατης ειτε διαχειρηστης. Καθως τα στοιχεια του αποθηκευνται στο αρχειο ``users.txt``.
  ...
  Η συναρτηση login επιτρεπει στον χρηστη να συνδεθει στον λογαργιασμο του ειτε ειναι πελατης ειτε ειναι admin. Καθως ψαχνει τα στοιχεια του στο αρχειο ``users.txt`` αν αυτα βρεθουν τοτε ο χρηστης συνδεθηκε στον λογαριασμο του με επιτυχια.
  ...
  Επισης η συναρτηση ``isAdminCheck`` ελεγχει καθε φορα αν ενας χρηστης ειναι admin ή πελατης ωστε να του δωθουν και οι αντιστοιχες λειτουργιες. Αυτο υλοποιηται ελεγχωντας καθε γραμμη απο τον φακελο ``users.txt`` το προτελευταιο χαρακτηρα αν ειναι 0 ή 1.
  ...
  Η επιλογη για την συνδεση ή την δημιουργια λογαργιασμου των χρηστων παρεχεται απο την συναρτηση ``startmenu`` 
...
Η παρακατω συναρτησης αφαιρουν τους κενους χαρακτηρες απο την αρχη και απο το τελος μιας συμβολοσειρας για την αποφυγη σφαλματων.
...
``
 ...
 H συναρτηση ``loadProductFromFile`` διαβαζει τα προιοντα απο το αρχειο και τα προσθετει σε ενα vector με το καταλληλο format.
...
``
    ...
    double price;
        ...
        product.DisplayProductInfo();   // εμφανιση της περιγραφης του προιοντος
    }
    // κλεισιμο αρχειου
    file.close();
    }
...
Εντος της ``main`` οριζουμε το vector ``products`` εισαγουμε τα προιοντα απο το αρχειο ``product.txt`` στο vector και εμφανιζουμε στον χρηστη για αρχη αν θελει να συνδεθει ή να δημιουργησει νεο λογαριασμο μεσω της συναρτησης ``startmenu``.
